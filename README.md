...
토큰 제한 때문에 기왕이면 한 서버에 하나의 봇만 사용하시는 것을 권장합니다.  
이 봇은 특정 사용자만 사용할 수 있도록 제한되어 있습니다.  
개발자는 구독제 플랜과 API 플랜이 따로 적용되는 사실을 깨닫고 개발을 멈췄습니다.(작동은 할거임...)  
...
- 봇은 `ALLOWED_SERVER_IDS`로 지정된 서버에서만 작동합니다.
- `ALLOWED_USER_IDS`에 명시된 사용자만 봇과 상호작용할 수 있습니다.
- 허용된 사용자는 Discord 채널에서 "/nbz" 접두사로 메시지를 보내 봇과 대화할 수 있습니다.
  예: "/nbz 안녕하세요, 오늘 날씨 어때요?"
- '냥!' 모드 토글 명령어:
